...
exports.getOrders = async (req, res) => {
    const orderList = await Order.find()
      .populate("user", "name")
      .populate({ path: "orderItem", populate: "pizza" }).sort({ dateOrdered: -1 });
  ...
  exports.getOrder = async (req, res) => {
    const order = await Order.findById(req.params.id)
      .populate("user", "name")
      .populate({ path: "orderItem", populate: "pizza" });
  ...
  exports.createOrder = async (req, res) => {
    ...
    const totalPrices = await Promise.all(
      resolvedId.map(async (orderItemId) => {
        const orderItem = await OrderItem.findById(orderItemId).populate("pizza", "price");
        const totalPrice = orderItem.pizza.price * orderItem.quantity;
        return totalPrice;
      }))
